...
var warningAnalysesetting = {
    html: '<div class="warnMainquery">' +
            '<div class="warnTitleBox" style="width:380px; height:23px;border:1px solid #c0c0c0;" ><p class="warnTitle1" style="width:250px;text-align:left;padding-left:5px;float:left;">预警分析参数设置</p></div>' +
        ...
        $("#warn_query_marker").live("click", function () {
            switch (index) {
                case 0:
                    ...
                    //});
                    //circle&2014.09.09&24&116.52986059374999,37.070224367188004,0.7652476016890013
                    //假数据处理
                    (function (circleStr, tdata) {

                        var arr = circleStr.split('&');
                        var circleArr = arr[3].split(',');

                        var data = tdata.filter(function (item) {

                            var lon = Number(item.Lon) - Number(circleArr[0]);
                            var lat = Number(item.Lat) - Number(circleArr[1]);

                            var value = Math.abs(lon * lon + lat * lat);
                            return value <= Number(circleArr[2]);
                ...
                case 1:
                    ...
                    //假数据
                    //rect&2014.09.09&24&117.98753488285,116.28903292973,37.342285429691,36.087000273441

                    (function (rectStr, tdata) {

                        var arr = rectStr.split('&');
                        var rectArr = arr[3].split(',');

                        var data = tdata.filter(function (item) {

                            return (item.Lon >= rectArr[1])
                            && (item.Lon <= rectArr[0])
                            && (item.Lat >= rectArr[3])
                            && (item.Lat <= rectArr[2]);
        ...
        if (type == "circle") {
            ...
            (function (circleStr, tdata) {
                ...
                var data = tdata.filter(function (item) {
                    ...
                    var value = Math.pow(lon * lon + lat * lat, 0.5);
                    return value <= Number(circleArr[2]);
