...
var luobaoqu = {
    ...
    html: '<div class="histroyWarningMain">' +
                ...
                '<div class="hWarnRegion" style="height:200px;">' +
                        ...
                        '</table>' +
                    '</div>' +
                    '<div class="hWarning_TitleP"><div class="hw_steps_coin" id="hw_steps_coin1"></div><div class="hWarn_Name" style="margin-left:6px;">历史查询</div></div>' +
    ...
    queryLastFive: function () {
        ...
        $.post(baseUrl + "HistoryWarning/HistoryWarningQueryLastFive", null, function (cbdata) {
            ...
            for (var i = 0; i < 5 && i < cbdata.length; i++) {
                ...
                hWarnRes = hWarnRes + '<tr><td>' + (i + 1) + '</td><td>' + cbdata[i].warningTime.substr(0, 16) + '</td><td>' + cbdata[i].operateTime.substr(0, strLength - 3) + '</td><td style="color:#026ba6;" class="lBqLast">查询</td></tr>';
            }
            $("#lbqResTable>tbody").html(hWarnRes);
            obj.lastHistroyEvt(lastHistoryWarningFive);
            //给每一行注册事件
            //$("#lbqResTable>tbody>tr").click(function () {
            //    var index = $("#lbqResTable>tbody>tr").index(this);
    ...
    lastHistroyEvt: function (lastHistoryWarningFive) {
        var obj = this;
        $(".lBqLast").click(function () {
            var index = $(".lBqLast").index(this);
            //叠加
            obj.overlayWarningResultLayer(index, lastHistoryWarningFive);
            //
            lbqResultAnalysis.init(R2.mapVector.gdbps);
    ...
    hWarnbtnClickEvt: function () {
        ...
        $.post(url, data, function (res) {
            ...
            var result = new luobaoquShow(res);
