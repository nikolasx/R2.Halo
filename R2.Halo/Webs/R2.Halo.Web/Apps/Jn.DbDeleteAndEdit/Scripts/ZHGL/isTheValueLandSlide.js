...
R2.Business.LandSlideQuery = OpenLayers.Class({
    ...
    mergeComprehensiveAndLandSlide: function () {
        ...
        $.post(baseUrl + "LandSlideImport/LandSlideImport", { "userInfo": "0", "compreStr": JSON.stringify(comprehensiveObj), "objStr": JSON.stringify(landslideObj), "planImgs": JSON.stringify(this.planImg) }, function (data) {
            if (data == "success") {
                alert("数据插入成功！");
            } else if (data == 2) {
                alert("此统一编号已经存在！");
