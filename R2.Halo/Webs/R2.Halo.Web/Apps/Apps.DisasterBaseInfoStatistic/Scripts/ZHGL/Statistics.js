$(function () {
    ...
    function postXianName(xianName_str, zhName_str, zhName) {
        ...
        //前台假数据模拟
        (function (cbdata, regionCode, disaType) {
            var data = [];
            regionCode.split(',').forEach(function (eachRegion) {
                var disTypes = disaType.split(',');
                var obj = {
                    regionCode: eachRegion,
                    list: new Array(disTypes.length)
                };
                disTypes.forEach(function (eachType, index) {
                    obj.list[index] = 0;
                    for (var i = 0; i < cbdata.length; i++) {
                        if (cbdata[i].统一编号.indexOf(eachRegion)
                            && cbdata[i].灾害类型 == getTypeNum(eachType)) {
                            obj.list[index]++;
                        }
                    }
                });
        ...
        })(disasterFakedData, xianName_str, zhName_str);

        function getTypeNum(disaType) {
            switch (disaType) {
                case "滑坡":
                    return "01";
                case "崩塌":
