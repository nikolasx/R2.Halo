...
function addqcqfcheckworker() {
    if ($("#qcqf_name").val() != "") {
        if ($("#editor_qcqf").hasClass("noeditorOperating")) {  //录入
            ...
            worker.Apartment = $("#qcqf_apartment").val();
            worker.Position = $("#qcqf_position").val();
            worker.Name = $("#qcqf_name").val();
            worker.Telephone = $("#qcqf_tele").val();
            worker.Beizhu = $("#qcqf_bz").val();
            $.post(baseUrl + "QcqfRecWorker/AddQcqfCheckWorker", { "worker": JSON.stringify(worker) }, function (data) {
                if (data == "success") {
                    alert("添加成功");
                }
            });
            ...
            worker.Telephone = $("#qcqf_tele").val();
            worker.Beizhu = $("#qcqf_bz").val();
            $.post(baseUrl + "QcqfRecWorker/UpdataQcqf", { "qcqf": JSON.stringify(worker) }, function (data) {
                if (data == "success") {
                    alert("编辑修改成功");
                }
                else {
...
R2.Business.GetFirstdata = OpenLayers.Class({
    ...
    GetPage: function () {
        var that = this;
        $.post(baseUrl + "QcqfRecWorker/GetAllDisaNum", function (data) {
            that.totalItem = data;
    ...
    pageInfoCss: function () {
        this.pageTotal = Math.ceil(this.totalItem / this.pageSize);
    ...
    EditorQcqf: function () {
        ...
        $(".qcqf_td_delete").live("click", function () {
            ...
            if (con == true) {
                ...
                $.post(baseUrl + "QcqfRecWorker/DeleteById", { "id": id }, function (data) {
                    if (data == "success") {
                        alert("删除成功！");
        ...
        $(".qcqf_td_editor").live("click", function () {
            var id = $(this).parent().parent().find(".qcqf_id").html();
            $.post(baseUrl + "QcqfRecWorker/GetdataById", { "id": id }, function (data) {
                $("#qcqf_txt_id").val(data.ID)
                var idx = getSelectIndex(data.Region);
                $("#qcqf_xian").get(0).selectedIndex = idx;
                $("#qcqf_apartment").val(data.Apartment);
                $("#qcqf_position").val(data.Position);
...
function GetAlldata(pageSize, pageIndex) {
    $.post(baseUrl + "QcqfRecWorker/GetAllDisaData", { "pageSize": pageSize, "pageIndex": pageIndex }, function (data) {
        if (data.length > 0) {
            var option = "";
            for (var i = 0; i < data.length; i++) {
                option += '<div class="qcqf_tr" >' +
                        ...
                        '<div class="qcqf_td" style="width:310px;">' + data[i].Apartment + '</div>' +
                        '<div class="qcqf_td">' + data[i].Position + '</div>' +
                        '<div class="qcqf_td2">' +
...
R2.Business.GetdataByKey = OpenLayers.Class({
    ...
    GetPage: function () {
        var that = this;
        $.post(baseUrl + "QcqfRecWorker/GetDataNumByKey", { "key": that.keyword }, function (data) {
            that.totalItem = data;
            that.pageTotal = Math.ceil(data / that.pageSize);
            that.pageInfoCss();
        });
...
function GetdataByKey(key, pageSize, pageIndex) {
    $.post(baseUrl + "QcqfRecWorker/GetDataByKey", { "key": key, "pageSize": pageSize, "pageIndex": pageIndex }, function (data) {
        if (data.length > 0) {
            var option = "";
            for (var i = 0; i < data.length; i++) {
                option += '<div class="qcqf_tr" >' +
