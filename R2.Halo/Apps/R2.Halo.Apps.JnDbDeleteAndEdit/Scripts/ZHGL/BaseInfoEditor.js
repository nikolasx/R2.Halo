...
function GetAlldata(type, leve) {
    bie_bool = true;
    totalItem = disasterFakedData.length;
    pageTotal = Math.ceil(totalItem / pageSize);
    pageIndex = 1;

    d = disasterFakedData.slice((pageIndex - 1) * pageSize, pageSize * pageIndex);
    if (d.length < 1) {
        alert("没有符合条件的内容！");
        ClearInfoData();
        return false;
...
function DoFirstselect() {
    ...
    $("#Submit").live("click", function () {
        ...
        if (bie_Keyword == "输入灾害点编号或名称") {
            ...
            pageTotal = Math.ceil(totalItem / pageSize);
            pageIndex = 1;
            d = disasterFakedData.slice(9 * pageSize, pageSize * 10);
            if (d.length < 1) {
                alert("没有符合条件的内容！");
                ClearInfoData();
                return false;
    ...
    $(".btneditor").live("click", function (evt) {
        ...
        switch (disasterCode) {
            ...
            case "01":
                ...
                window.location.href = (baseUrl1+"Apps/JnDbDeleteAndEdit/EditLandSlide?landSlideId=" + locaTybh);
               
                break;
                //崩塌
            case "02":
                window.location.href = (baseUrl1 + "Apps/JnDbDeleteAndEdit/EditCollapse?collapseId=" + locaTybh);
                break;
            case "03":
                window.location.href = (baseUrl1 + "Apps/JnDbDeleteAndEdit/EditMudFlow?mudFlowId=" + locaTybh);
                break;
            case "04":
                window.location.href = (baseUrl1 + "Apps/JnDbDeleteAndEdit/EditGroundSubside?groundSubsideId=" + locaTybh);
                break;
            case "05":
                window.location.href = (baseUrl1 + "Apps/JnDbDeleteAndEdit/EditGroundSettle?groundSettleId=" + locaTybh);
                break;
            case "06":
                window.location.href = (baseUrl1 + "Apps/JnDbDeleteAndEdit/EditLandCrack?landCrackId=" + locaTybh);
                break;
            default:
                break;
