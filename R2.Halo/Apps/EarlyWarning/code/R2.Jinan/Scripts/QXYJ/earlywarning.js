...
function EarlyWarning(warningTime, gridClecked, yjRainChecked, outres, isjump, wrlayer) {
    ...
    (function () {
        ...
        if (EarlyWarningStepLog <= 7) {

            window.time = setTimeout(EarlyWarning, 5000);
...
function CreateImgJpg(warntime, opretime) {
    ...
    $("#createJpg").live('click', function() {
        ...
        //window.open(baseUrl + "EarlyWarning/CreatRfLayerImg?" + paramUrl);
        $("#downYjImg").remove();
        $("body").append("<a id='downYjImg' ></a>");
        $("#downYjImg").attr("href", window.baseUrl1 + "EarlyWarning/DownImg");
        $("#downYjImg").get(0).click();
    });
}
//预览切换
function ScanJpg(warntime, opretime) {
    ...
    //})
    $('#scanJpg').toggle(function () {
        //$(this).toggle(function () { showScanContent(warntime, opretime); },
        // function () { removeScanLayers(); $('#scanJpg').html("产品预览"); }).trigger('click');;

        //window.open(window.baseUrl + "Content/images/img/济南预警图.jpg");

        $("#imgylt").remove();
        $("#fullMap").append('<div id="imgylt"><img/></div>');
        ...
        $("#imgylt img").attr("src", window.baseUrl + "Content/images/img/济南预警图.jpg");
