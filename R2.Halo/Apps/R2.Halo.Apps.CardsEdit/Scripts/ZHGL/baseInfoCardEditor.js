...
R2.Business.workCardResult = OpenLayers.Class({
    ...
    searchByKeyWord: function () {
        ...
        if ($("#txtValue").val().toString().trim() != "输入灾害点编号或名称") {
            ...
            //    tempObj.pageCss();
            //})
            (function (data, pageIndex, pageSize, keyWord) {

                var cbdata = data.filter(function (item) {
                    return item.地理位置.contains(keyWord) || item.名称.contains(keyWord);
                });

                var cbdata = data.slice((pageIndex - 1) * pageSize, pageIndex * pageSize);

                tempObj.addHtml(cbdata);
                tempObj.pageCss();

            })(disasterFakedData, tempObj.pageIndex, tempObj.pageSize, $("#txtValue").val().toString().trim());

            (function (data) {

                $(".allPage").text("第1/" + Math.ceil(data.length / 10) + "页");
                tempObj.totalCount = data.length;
    ...
    getData: function () {
        ...
        })(disasterFakedData, tempObj.pageIndex, tempObj.pageSize);
    ...
    editorNum: function (Mydata) {
        ...
        (function (data1, data2, tids) {
            ...
            for (var i = 0; i < tidsArr.length; i++) {
                ...
                var obj = {
                    PreventNum: data1.filter(function (item) {
                        return item.统一编号 == tidsArr[i];
                    }).length,
                    HedgeNum: data2.filter(function (item) {
                        return item.统一编号 == tidsArr[i]
                    }).length
                };
        ...
        })(preventCards, RiskCards, tybhData);
    },
    //点击明白卡数量展开明白卡
    openData: function () {
        var thisobj = this;
            ...
            if ($(this).parent().next(".openField").hasClass("openFieldOpen openFieldOpenSlide")) {
                ...
                if ($(this).text() == "0") {
                    ...
                    if ($(this).parent().next(".openField").hasClass("openFieldOpen")) {
                        ...
                        (function (fakedata, bh, pageSize, pageIndex) {

                            var tdata = fakedata.filter(function (item) {
                                return item.统一编号 == bh;
                            });

                            data = tdata.slice((pageIndex - 1) * pageSize, pageIndex * pageSize);
    ...
    showWorkData: function () {
        $(".openField").children(".openFieldLi").click(function () {
            ...
            fullPanel2.showByIFrame(baseUrl1 + "Apps/DisasterCard/EditWorkCard?editWorkCardId=" + $(this).children("input").val());
        });
    },
    showHedgeData: function () {
        $(".openField2").children(".openFieldLi").click(function () {
            fullPanel2.showByIFrame(baseUrl1 + "Apps/DisasterCard/EditEscapseCard?editHedgeCardId=" + $(this).children("input").val());
        });
    },
    pageCss: function () {
        if (this.pageIndex == 1) {
    ...
    showHedgeDataByPage: function (tempObj, tybh) {
        ...
        (function (cbdata, tid, pageSize, pageIndex) {

            var data = cbdata.filter(function (item) {
                return item.统一编号 == tid;
            });
        ...
        })(RiskCards, tybh, thisobj.pageSize2, thisobj.pageIndex2);
    },
