...
function GetAlldata(type, leve) {
    $.post(baseUrl + "PreCautionPlans/GetallDisCount", { "DisaType": type, "DisaGrade": leve }, function(data) {
        bie_bool = true;
        totalItem = data;
        pageTotal = Math.ceil(totalItem / pageSize);
        pageIndex = 1;
        $.post(baseUrl + "PreCautionPlans/GetallDis", { "DisaType": type, "DisaGrade": leve, "pageSize": pageSize, "pageIndex": pageIndex }, function(d) {
            if (d.length < 1) {
                alert("没有符合条件的内容！");
                ClearInfoData();
                return false;
...
function DoFirstselect() {
    ...
    $("#Submit").live("click", function() {
        ...
        if (bie_Keyword == "输入灾害点编号或名称") {
            ...
            bie_disaType = checkBoxConInfoLx("#bit_zhxl");
            bie_disaGrade = checkBoxConInfoDj("#dangerClassDmtx");
            $.post(baseUrl + "PreCautionPlans/GetDisaCountByCondition", { "key": bie_Keyword, "DisaType": bie_disaType, "DisaGrade": bie_disaGrade }, function(data) {
                totalItem = data;
                pageTotal = Math.ceil(totalItem / pageSize);
                pageIndex = 1;
                $.post(baseUrl + "PreCautionPlans/GetDisaByCondition", { "key": bie_Keyword, "DisaType": bie_disaType, "DisaGrade": bie_disaGrade, "pageSize": pageSize, "pageIndex": pageIndex }, function(d) {
                    if (d.length < 1) {
                        alert("没有符合条件的内容！");
                        ClearInfoData();
                        return false;
...
function Search() {
    if (bie_bool) {
        $.post(baseUrl + "PreCautionPlans/GetallDisCount", { "DisaType": bie_disaType, "DisaGrade": bie_disaGrade }, function (data) {
            totalItem = data;
            pageTotal = Math.ceil(totalItem / pageSize);
            $.post(baseUrl + "PreCautionPlans/GetallDis", { "DisaType": bie_disaType, "DisaGrade": bie_disaGrade, "pageSize": pageSize, "pageIndex": pageIndex }, function (d) {
                if (d.length < 1) {
                    alert("没有符合条件的内容！");
                    ClearInfoData();
                    return false;
...
function pageMana() {
    ...
    $(".btndelete").live("click", function(evt) {
        ...
        var com = confirm("你确定要删除名称为" + locaName + "的防灾预案信息吗？");
        if (com == false) {
            return;
        }
        $.post(baseUrl + "PreCautionPlans/DeletPrecautionPlanByTid", { "id": locaTybh }, function(data) {
            if (data > 0) {
                alert("删除成功！");
                Search();
            } else {
    ...
    $(".btneditor").live("click", function(evt) {
        var locaTybh = $(this).parent().parent().find(".dataline_class").eq(1)[0].innerHTML;
        top.fullPanel2.showByIFrame(baseUrl + "PreCautionPlans/EditorPrecautionPlan?tid=" + locaTybh);
    });
